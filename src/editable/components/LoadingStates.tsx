...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[1.6rem] bg-[var(--slot4-page-text)]/8', className)} />
}
...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--slot4-page-text)]/45">{label}</p>
      <PulseBlock className="mt-5 h-16 w-3/4 max-w-4xl rounded-[2rem]" />
      <PulseBlock className="mt-4 h-6 w-2/3 max-w-2xl" />
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[2rem] border border-[var(--editable-border)] bg-white/70 p-5">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[1.85rem] border border-[var(--editable-border)] bg-white/72 p-4">
          <PulseBlock className="h-48 w-full" />
...
export function DetailLoadingState({ label = 'Loading detail', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto grid w-full max-w-[var(--editable-container)] gap-8 px-4 py-12 lg:grid-cols-[0.8fr_1.2fr]', className)} aria-live="polite" aria-busy="true">
      <PulseBlock className="h-96 w-full rounded-[2.4rem]" />
      <div>
        <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--slot4-page-text)]/45">{label}</p>
