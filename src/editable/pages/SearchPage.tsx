...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-[0_20px_60px_rgba(16,35,61,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_80px_rgba(16,35,61,0.14)] ${strong ? 'md:col-span-2' : ''}`}>
      <div className={`relative overflow-hidden bg-[var(--slot4-media-bg)] ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[rgba(16,35,61,0.78)] via-[rgba(16,35,61,0.08)] to-transparent" />
        <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--slot4-page-text)]">{taskLabel}</span>
      </div>
      <div className="p-5 sm:p-6">
        <h2 className="mt-1 line-clamp-3 text-2xl font-black leading-[0.95] tracking-[-0.06em] text-[var(--slot4-page-text)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.6rem] border border-[var(--editable-border)] bg-white/82 p-6 shadow-[0_24px_80px_rgba(16,35,61,0.08)] backdrop-blur lg:grid-cols-[0.84fr_1.16fr] lg:p-10">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-[var(--slot4-accent-fill)]">{pagesContent.search.hero.badge}</p>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-[#f8fbfe] p-4 sm:p-5">
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--slot4-page-text)]/45">{results.length} results</p>
              <h2 className="mt-2 text-3xl font-black tracking-[-0.06em]">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
            <Link href="/pdf" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black">Browse PDFs <ArrowRight className="h-4 w-4" /></Link>
          </div>
