...
export default function CommentsPage() {
  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
        <section className="rounded-[2.4rem] border border-[var(--editable-border)] bg-white/82 p-6 shadow-[0_24px_80px_rgba(16,35,61,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.24em] text-[var(--slot4-accent-fill)]">
              ...
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--slot4-muted-text)]">Review comments saved in this browser from article pages.</p>
            </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--slot4-page-text)]/45" />
              <input
                value={query}
                onChange={(event) => {
                ...
                className="h-12 w-full rounded-[1.2rem] border border-[var(--editable-border)] bg-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.9rem] border border-[var(--editable-border)] bg-white/82 p-5 shadow-[0_20px_60px_rgba(16,35,61,0.06)]">
          ...
          <section className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/70 p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.7rem] border border-[var(--editable-border)] bg-white/82 p-4 text-sm text-[var(--slot4-muted-text)]">
